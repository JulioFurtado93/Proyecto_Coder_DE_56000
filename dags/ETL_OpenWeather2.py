...
with open(dag_path+'/keys/'+"pwd.txt",'r') as f:
    pwd= f.read()
with open(dag_path+'/keys/'+"app_pwd.txt",'r') as f:
    app_pwd= f.read()
...
def extraer_data(exec_date):
    try:
        ...
        print("El formato datetime deberia ser %Y-%m-%d %H", e)
        raise e       

# Funcion de transformacion en tabla
def transformar_data(exec_date,**kwargs):       
    ...
    #obtener temp para threshold
    temp_value = sorted_weather_df['temp'][0]
    kwargs['ti'].xcom_push(key='temp', value=temp_value)
...
#Funcion de alerta por mail
def envio_mail(**kwargs):
    threshold = kwargs['ti'].xcom_pull(task_ids='task_2', key='temp')
    add = ''

    if threshold is not None:
        if threshold < 30:
            add = 'Nivel verde, no hay ninguna alerta. No hay peligro sobre la salud de la poblacion'
        elif threshold < 35:
            add = 'Nivel amarillo, efecto leve a moderado. Puede ser peligroso para infantes y mayores de 65 con enfermedades cronicas'
        elif threshold < 37:
            add = 'Nivel naranja, efecto moderado a alto. Puede ser muy peligroso para grupos de riesgo'
        else:
            add = 'Nivel rojo, efecto alto a extremo. Puede afectar a todas las personas, incluso las saludables'

    try:
        x=smtplib.SMTP('smtp.gmail.com',587)
        x.starttls()
        x.login('juliocfurtado93@gmail.com',app_pwd)
        subject='ETL OpenWeather'
        body_text=f'El proceso ETL fue completado con exito. \n {add}'
        message=f'Subject: {subject}\n\n{body_text}'
        x.sendmail('juliocfurtado93@gmail.com','furiotattoo@gmail.com',message)
        print('Exito')
